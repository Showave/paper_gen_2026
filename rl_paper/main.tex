...
does not provide our conditional estimator identity. VCPO instead diagnoses
ESS collapse under asynchronous policy lag and adapts the learning rate while
using an off-policy variance-reducing baseline \cite{huang2026vcpo}. It is a
strong stability/throughput comparator, but changing the transformed optimizer
step is distinct from auditing an unbiased raw estimator at one frozen policy.
Statistically, the design is a restricted stratified multiple-importance
sampler with augmentation.
...
At the artifact boundary, each successful current row stores projected $A$,
each stale row stores projected $B$, and each reference row stores projected
$U_\lambda s$. The aggregator must reconstruct
$\overline A+(1-\alpha)\overline B$; a generic field called ``gradient'' is
insufficient. Every attempted row also retains allocated accelerator-seconds,
including failed and reference draws. Any infrastructure failure blocks
estimator-claim eligibility rather than allowing a success-conditioned MSE.
This projected-term schema is only an intermediate audit. Real execution
remains blocked until a reviewed validator reconstructs $U,h,w,A,$ and $B$
from hash-bound prompt/decoder probabilities, rewards, lengths, the frozen
$\lambda$, nuisance predictions, and projected scores; self-declared terms are
...
hidden-evaluator leakage analysis. Above the exhaustive limit, only a reviewed
exact engine whose hash-bound shards cover every cross-record field segment
can lift the lexical blocker. The committed engine is unfrozen and indexes
record pairs only, so it cannot clear a real multi-field export.
...
\textbf{H1}, the selected exact adaptive-mixture estimator lowers
projected-MSE--cost risk relative to an on-policy control on untouched
confirmation draws; \textbf{H2}, any
...
Intervals jointly resample paired candidate replication IDs across cells and
independently resample $R_1$ and $R_2$ trajectories, retaining shared-reference
dependence. Every draw recomputes measured cost, nonnegative MSE--cost risk,
and selected-minus-on-policy confirmation contrasts.
Point estimates are independently recomputed from the hash-bound trajectory
ledger: candidate stratum means, MSE, mean complete-replication cost, the
selection argmin, and both selected and on-policy untouched confirmation cells
must all reproduce.
Reference-generation cost is reported separately rather than disappearing as a
shared constant.
...
bias, variance, MSE, interval coverage, residual-weight ESS by source, and
...
times measured accelerator-seconds at common 64-trajectory and verifier-call
counts. Because the unbiased cross-reference MSE can be negative, selection
uses $\max(\widehat{\operatorname{MSE}}_\times,0)$ times mean
complete-replication accelerator-seconds while retaining the raw estimate;
ties choose lower cost, then larger $\alpha$. The audit freezes $\lambda=0$
and counts EOS as a response token. Untouched confirmation includes the
selected cell and the $\alpha=1$ control under $D_{\rm confirm}$.
Tying mixture mass to allocation fraction is a pilot restriction, not
...
frozen estimator-pilot seeds. End-to-end development and confirmatory seed
counts remain planned.
...
\begin{table*}[t]
\caption{Planned primary results. Higher held-out reward is better;
lower projected gradient MSE, MSE--cost risk, and generated tokens to threshold are better.
...
\begin{tabular}{lcccc}
\toprule
Method & \shortstack{Projected\\MSE / risk}
& \shortstack{Held-out\\reward}
& \shortstack{Tokens to\\threshold}
& \shortstack{Mean response\\tokens} \\
\midrule
On-policy RLOO & -- & -- & -- & -- \\
Bounded mixture IS & -- & -- & -- & -- \\
...
Estimator-claim eligibility records assumption and artifact checks; it is
separate from the selected-versus-on-policy H1 estimate, interval, and result.
...
normalization; residual-weight ESS by source; maximum weight and support
...
miss pairs, it is inconclusive by itself.
\texttt{scripts/validate\_external\_overlap\_audit.py} admits a large-artifact
result only after identifier-ordered, row-major upper-triangle shard ranges
cover exactly $n(n-1)/2$ comparisons with no gap or overlap. The manifest binds
the materialization, gate configuration, reviewed exact-engine source, runtime
image, and review record; approximation, early termination, shard errors, and
qualifying pairs are blocking. The validator separately checks every reported
pair's ordinal, identifiers, roles, threshold, and score while retaining no
matched text. A temporary disk-backed exact join also enumerates every pair
sharing a 5-gram and rejects omissions; this is recall-complete for the
positive frozen Jaccard thresholds because any qualifying pair must share a
gram. Shard coverage is an engine-governance attestation, whereas readiness
depends on reproducing the complete qualifying-pair set. Its committed engine
hashes are null blockers, so only the generated contract self-test can
currently pass. Even an exhaustive lexical pass neither clears a source nor
establishes semantic independence from a hidden evaluator.
The external contract indexes record pairs rather than canonical field
segments, so it cannot clear a multi-field export even after its engine fields
...
protected-input, and protected-target slots where applicable. Its validator
binds each inventory to source revision, configuration, split, exporter,
canonical paths, and a review-artifact hash. Runtime enumeration requires
every textual leaf to be projected or explicitly excluded, and
training/protected fields are compared even within one record. Generated
attestations cannot be relabeled as real. The schema audit covers the DAPO prompt and protected-answer classes,
but the actual field inventory and review record remain null blockers.
...
\cref{tab:main}.
\texttt{scripts/aggregate\_rl\_estimator.py} provides a second, artifact-level
audit. Its generated two-dimensional ledger has hand-computed current, stale,
and independent reference terms; the same code used for a real ledger must
recover both selection-cell MSEs, the nonnegative-risk argmin, and selected
plus on-policy confirmation MSEs. It jointly resamples complete paired
candidate replication IDs across cells and independently resamples $R_1$ and
$R_2$ trajectories within each target law. A domain-separated stream drives
exact nearest-rank 2.5/97.5 percentile bounds for cell MSE, measured cost,
risk, and selected-minus-control contrasts. The pilot refreezes a
feasible shape of 64 projected coordinates, 250 candidate replications per
cell, 2,048 trajectories per reference sample, and 2,000 bootstrap draws.
NumPy~2.5.3 PCG64 emits exact multinomial weights in chunks of 32; projected
means and squared norms avoid materializing resampled trajectory tensors, and
the self-test executes the full selection-law shape. Tampered MSE or intervals
and incomplete source strata must fail; infrastructure failure must block
estimator-claim eligibility. The fixture is marked non-empirical and cannot
populate any result cell. The script still consumes projected terms
rather than reconstructing $U,h,w,A,$ and $B$ from raw probabilities and
outcomes; that raw-field gap, the unreviewed field inventory, and three pending
fault injections keep the artifact-integrity gate closed.
