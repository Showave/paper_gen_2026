...
importance ratio, and frozen reward and length models supply an
augmented control variate; under explicit independence, support, and decoding
assumptions, the resulting one-update gradient estimator is conditionally
unbiased. A Lagrangian expected-response-length constraint makes length part of the
declared objective rather than an implicit per-token normalization. This
...
$t$, a live stale actor or sealed queue produces \emph{new}, never-consumed
...
$D$ with density $d(x)$, and the exact rollout distribution is
...
\end{equation}
$G_t$ is a live actor or a sealed, never-used queue; a reward-selected replay
buffer does not satisfy the core assumptions.
...
$t$, then frozen. A convenient adaptive proposal is
\begin{equation}
q_t(x)=\epsilon d(x)+(1-\epsilon)\overline q_t(x),\qquad
\overline q_t(x)=
\frac{d(x)\sqrt{\widehat v_t(x)/\widehat c_t(x)}}
{\sum_{x'}d(x')\sqrt{\widehat v_t(x')/\widehat c_t(x')}},
\end{equation}
...
batch; collection never stops early because a realized token total is reached.
...
require joint inclusion probabilities.
