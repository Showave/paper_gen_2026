...
including failed and reference draws. Any infrastructure failure blocks H1
aggregation rather than allowing a success-conditioned MSE.
...
selection argmin, and the one untouched confirmation cell must all reproduce.
...
recover both selection-cell MSEs, their measured-cost argmin, and the
confirmation MSE. Tampered MSE, an incomplete source stratum, and an
infrastructure failure are injected and must respectively fail validation or
block H1. The fixture is marked non-empirical and cannot populate any result
cell.
