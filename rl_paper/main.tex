...
exact engine whose hash-bound shards cover every unordered pair can lift the
lexical blocker; the committed engine attestation is still unfrozen.
...
plus on-policy confirmation MSEs. Tampered MSE, an incomplete source stratum, and an
infrastructure failure are injected and must respectively fail validation or
block estimator-claim eligibility. The fixture is marked non-empirical and
cannot populate any result cell. The script still consumes projected terms
rather than reconstructing them from raw probabilities and outcomes, and it
does not independently reproduce the three-way bootstrap; the
artifact-integrity gate therefore blocks all real execution.
That gate also requires reviewed source/role projection schemas covering every
model-visible and protected textual field; the current single canonical
deduplication projection cannot establish hidden-answer or test independence.
