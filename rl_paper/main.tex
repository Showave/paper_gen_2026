...
binds each inventory to a source revision, exporter implementation, canonical
field paths, and review record; generated attestations cannot be relabeled as
real. The schema audit covers the DAPO prompt and protected-answer classes,
...
plus on-policy confirmation MSEs. It independently resamples complete
candidate replications, $R_1$ trajectories, and $R_2$ trajectories within each
target law, derives a domain-separated stream from the frozen seed, and
requires exact nearest-rank 2.5/97.5 percentile bounds. The pilot refreezes a
