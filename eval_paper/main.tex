...
family-variance component. Human-rater effects are partially pooled under a
proper prior and do not require every rater to appear in the 200-attempt pilot.
...
r_b(x)&=\exp\{g_b(x)\}/\sum_{x'}\exp\{g_b(x')\},\\
...
correct posterior, exact update, and no exploration constraint; the implemented
approximation claims neither finite-horizon optimality nor a general regret
bound. It scores approximate Bayesian risk
...
DGP and policy implementations remain hash-blocked, so this frozen layout is
...
Efficiency will be claimed only if interval validity is retained in simulation
and an interval for the adaptive-minus-audit difference lies within a frozen
equivalence margin using a frozen joint-covariance procedure. Neither is yet
registered, so the current draft permits no equivalence claim. Jury-only
...
crossing with a deterministic 189-condition fractional manifest. Its validator
...
but outcomes from shared raters may be correlated and are not treated as
independent in a future contrast.
...
equivalence test remains blocked until its margin and shared-rater covariance
