...
The design claim additionally requires stability of the full response law:
\begin{equation}
P(Y_t=y\mid\mathcal F_{t-1},X_t=x)=p_y(x),
\quad y\in\{-1,0,1\},
...
$R_t$ indicate an observed rating and set the recorded $Y_t^{\rm obs}=0$ when
$R_t=0$. The primary report replaces a missing outcome by $-1$ and $+1$ in
\begin{align}
\psi_t^\pm
...
\end{align}
We apply one-sided martingale bounds to the two endpoint averages. A
separate MAR analysis includes the logged response propensity; it is not the
...
The primary interval is two-sided for $\Delta_F$, with the frozen
...
every skip or nonresponse is retained.
...
validator and stage contract; site-local commands are required for execution,
after which output hashes and costs are recorded.
