...
hypothetical EVI update by one frozen Laplace/Newton step, forms one $p_t$ for
the next batch, and logs that probability for every draw. Conditional on the
batch-start history, draws are IID with replacement from this frozen law and
the predictor is fixed; draw-level \cref{eq:psi} remains valid because both are
predictable.
...
Let $a(\mathcal F)$ apply the confidence-set rule in
...
replacements. At an intermediate history, the working posterior defines the
risk if labeling stopped now,
$R(\mathcal F)=\mathbb E[L\{a(\mathcal F),S(\Delta_F)\}\mid\mathcal F]$.
