...
replacements. At batch start $b$, let $Q_b$ be the frozen working posterior
over states $S\in\{A,B,E\}$. Define its Bayes risk
\begin{equation}
\widetilde R(Q)=
...
$U(Q_b;x,y)$ be the posterior obtained by the predeclared single
Laplace/Newton update for hypothetical outcome $y$; neither the update nor its
hyperparameters depend on $p_b$. For target unit $x$ with annotation cost
$k_x$, define
\begin{align}
...
withdrawals, and response are reported. A binding operational quota would
destroy per-draw positivity for that rater and therefore halts the
point-identified primary analysis rather than triggering silent reassignment.
The report then gives only the predeclared partial-identification bounds.
...
cost, nonresponse rule, and full simulation grids. These values specify a
...
seed, assignments, and outcomes are hidden. The streams are independent
conditional on the common frozen frame.
